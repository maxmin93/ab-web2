...
export class StatGraphComponent implements OnInit {
  ...
  cyCanvasCallback():void {
    gtag('event', 'tap_canvas', {'event_category':'graph', 'event_label':'meta'});
  ...
  cyElemCallback(target:any):void {
    gtag('event', 'tap_element', {'event_category':'graph', 'event_label':'meta'});
  ...
  onSelectProperty({ selected }) {
    ...
    gtag('event', 'tap_property', {'event_category':'graph', 'event_label':'meta'});
  ...
  loadMetaGraph(gid: number){
    ...
    data$.pipe( filter(x => x['group'] == 'nodes') ).subscribe(
      (x:INode) => {
        ...
        // **NOTE: 이미 가지고 있는 label 에 대해 스타일 정보 복사
        //        after import, we have some problems about style
        this.labels
    ...
    data$.pipe( filter(x => x['group'] == 'end') ).subscribe(
      (x:IEnd) => {
        this._util.calcElementStyles( this.statGraph.nodes, (x)=>30+x*3, false );
