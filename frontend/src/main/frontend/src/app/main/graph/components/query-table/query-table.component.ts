...
export class QueryTableComponent implements OnInit, AfterViewInit {
  ...
  onSelect( event ) {
    // console.log('Event: select', this.recordColumns, event, this.selected);
  }

  // **NOTE: onSelectCell 이 작동하지 않음. activate 이벤트에서 click 이벤트 필터링 해서 사용
  onSelectClick( event ){
    if( event.type != 'click' ) return;
    gtag('event', 'tap_cell', {'event_category':'graph', 'event_label':'table'});

    let index = event.cellIndex > 0 ? 
              (event.cellIndex > this.recordColumns.length ? this.recordColumns.length : event.cellIndex) - 1
              : 0;
    this.openBottomSheet(this.recordColumns[index], event.row[index]);
  }
  ...
  openBottomSheet(col:any, cell:any): void {
