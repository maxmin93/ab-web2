...
export class QueryGraphComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    let title_shown = localStorage.getItem(CONFIG.TITLE_SHOWN_KEY);
    ...
    this.cy = agens.graph.graphFactory(
      this.divCanvas.nativeElement, {
        ...
        hideNodeTitle: title_shown == 'true' ? false : true,    // hide nodes' title
  ...
  handleKeyUpEvent(event: KeyboardEvent) { 
    ...
    if (this.canvasHover && event.ctrlKey) {
      gtag('event', 'shortcut', {'event_category':'graph', 'event_label':'canvas', 'value': charCode});
  ...
  toggleEditMode(checked?:boolean){
    gtag('event', 'btn_edit', {'event_category':'graph', 'event_label':'toolbar'});
    ...
    if( this.btnStatus.editMode ){      // editMode on
      ...
      // **NOTE: 남아 있는 빨간점(.eh-handle) 노드 제거 
      setTimeout(()=>{this.cy.remove('.eh-handle');},10);
    }
  }
  ...
  cyCanvasCallback(e):void {
    gtag('event', 'tap_canvas', {'event_category':'graph', 'event_label':'canvas'});
  ...
  cyElemCallback(target:any):void {
    gtag('event', 'tap_element', {'event_category':'graph', 'event_label':'canvas'});
  ...
  qtipEleMenu( action, targets ){
    gtag('event', 'tap_elemenu', {'event_category':'graph', 'event_label':'canvas'});
  }

  // qtipMenu 선택 이벤트
  qtipCxtMenu( action ){
    gtag('event', 'tap_cxtmenu', {'event_category':'graph', 'event_label':'canvas'});
    ...
    switch( action )    {
      ...
      case 'cut': 
              ...
              break;
      case 'paste': 
              if( targets.empty() ) this.ur.do('paste', this.updateGraph);
              break;
      case 'remove': 
  ...
  isDirty():boolean {
    return this.cy.elements().size() > 0 ? true : false;
  }
  ...
  refreshLabels(){
    // label 정렬 : node>edge 순으로, size 역순으로
    this.labels = [... _.orderBy(this.labels, ['type','size'], ['desc','desc'])];    
  }
  ...
  clickGraphLabelChip( label:ILabel ): void {
    gtag('event', 'tap_label', {'event_category':'graph', 'event_label':'canvas'});
  ...
  graphChangeLayout(layout:string){
    if( layout != 'random' ) 
      gtag('event', 'btn_layout', {'event_category':'graph', 'event_label':'toolbar', 'value': layout});
  ...
  toggleShowHideTitle(checked?:boolean): void{
    gtag('event', 'btn_title', {'event_category':'graph', 'event_label':'toolbar'});
    ...
    // get server setting of title_shown
    let title_shown = localStorage.getItem(CONFIG.TITLE_SHOWN_KEY);
    if( title_shown != 'true' ){
        // graph의 userZoomingEnabled 설정 변경
  ...
  toggleHighlightNeighbors(checked?:boolean): void{
    gtag('event', 'btn_neighbor', {'event_category':'graph', 'event_label':'toolbar'});
  ...
  openSheetEditElement(element:any=undefined, callback:Function=undefined): void {
    ...
    bottomSheetRef.afterDismissed().subscribe((x) => {
      ...
      if( x.created ){
        ...
        if( x.updated_props.length > 0 ){
          this._api.grph_update_prop(this.gid, x.element.group, x.element.data.id, 'upsert', x.updated_props).subscribe(
            e => {
              // console.log( e );
            });
        }
        if( x.deleted_props.length > 0 ){
          this._api.grph_update_prop(this.gid, x.element.group, x.element.data.id, 'delete', x.deleted_props).subscribe(
            e => {
              // console.log( e );
            });
        }      
  ...
  openFilterNGroupSheet(): void {
    ...
    gtag('event', 'btn_filterNgroup', {'event_category':'graph', 'event_label':'toolbar'});
  ...
  graphCentrality(option:string='degree'){ 
    gtag('event', 'btn_centrality', {'event_category':'graph', 'event_label':'toolbar', 'value':option});
  ...
  toggleFindShortestPath(option:boolean=undefined){
    gtag('event', 'btn_spath', {'event_category':'graph', 'event_label':'toolbar'});
  ...
  toggleFindConnectedGroup(option:boolean=undefined){
    gtag('event', 'btn_component', {'event_category':'graph', 'event_label':'toolbar'});
  ...
  toggleFindCycles(option:boolean=undefined){
    gtag('event', 'btn_cycle', {'event_category':'graph', 'event_label':'toolbar'});
  ...
  toggleTimeline(option:boolean=undefined){
    gtag('event', 'btn_timeline', {'event_category':'graph', 'event_label':'toolbar'});
  ...
  reloadGraph(){
    ...
    gtag('event', 'btn_reload', {'event_category':'graph', 'event_label':'toolbar'});
  ...
  openImageExportDialog(){
    ...
    gtag('event', 'btn_image', {'event_category':'graph', 'event_label':'toolbar'});
