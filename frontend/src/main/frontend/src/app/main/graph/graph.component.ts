...
//  `;
  ...
  ngAfterViewInit() {
    gtag('set', {'page_title':'graph', 'page_path':'/graph', 'screen_name':'graph'});
    ...
    this.editor.on('keyup', (cm, e)=>{      
      if( e.keyCode == 13 ){
        setTimeout(function(){ 
          agens.cy.resize();
          // cm.resize();       // ERROR: cm.resize() is not a funciton
        }, 1);
      } 
    });
    // 
  ...
  tabAnimationDone(){
    switch( this.currentTabIndex ){
      case 0: 
          ...
          gtag('event', 'tap_graph', {'event_category':'graph', 'event_label':'tab_change'});
          break;
      case 1: 
          this.queryGraph.isVisible = false;
          this.statGraph.isVisible = false;          
          Promise.resolve(null).then(() => this.queryTable.refresh() ); 
          gtag('event', 'tap_table', {'event_category':'graph', 'event_label':'tab_change'});
          break;
      case 2: 
          this.queryGraph.isVisible = false;
          this.statGraph.isVisible = true;
          Promise.resolve(null).then(() => this.statGraph.refreshCanvas() ); 
          gtag('event', 'tap_meta', {'event_category':'graph', 'event_label':'tab_change'});
          break;
      default: 
          Promise.resolve(null).then(() => this.currentTabIndex = 0 );
          gtag('event', 'tap_blank', {'event_category':'graph', 'event_label':'tab_change'});
          break;
    }
  }
  ...
  toggleExpandEditor(){
    ...
    gtag('event', 'toggle_expand', {'event_category':'graph', 'event_label':'query'});
  }
  ...
  clearProject(option:boolean = true){
    gtag('event', 'new_query', {'event_category':'graph', 'event_label':'editor'});
  ...
  runQuery(option:boolean = false){
    gtag('event', 'run_query', {'event_category':'graph', 'event_label':'editor'});
  ...
  stopQuery(){
    gtag('event', 'stop_query', {'event_category':'graph', 'event_label':'editor'});
  ...
  openProjectSaveDialog(){
    gtag('event', 'save_project', {'event_category':'graph', 'event_label':'editor'});

    if( !this.resultGraph && !this.queryGraph.isDirty() ){
      this._api.setResponses(<IResponseDto>{
        group: 'project::save',
    ...
    if( sql.length < 5 ){
      this._api.setResponses(<IResponseDto>{
        group: 'project::save',
        state: StateType.WARNING,
        message: 'Query Editor is empty. At least fill editor any comment'
      });
      return;
    }
  ...
  openProjectOpenDialog(){
    gtag('event', 'open_project', {'event_category':'graph', 'event_label':'editor'});
  ...
  importFile(event){
    gtag('event', 'import_file', {'event_category':'graph', 'event_label':'editor'});
  ...
  exportGraph(fileType:string){
    gtag('event', 'export_file', {'event_category':'graph', 'event_label':'editor'});
