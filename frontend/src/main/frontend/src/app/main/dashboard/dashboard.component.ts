...
export class DashboardComponent implements AfterViewInit, OnDestroy {
  ...
  // call API
  handlers:Subscription[] = [undefined, undefined, undefined, undefined, undefined, undefined, undefined];
  dto_message: string = '';
  ...
  ngAfterViewInit() {
    gtag('set', {'page_title':'schema', 'page_path':'/schema', 'screen_name':'schema'});
    gtag('event', 'screen_view');

    // **NOTE: 왜인지 모르겠지만, this.cy 가 미처 할당 안된 상태로 시작. "cy.on()"에서 ERROR!!
    //        ==> promise.resolve 로 응급조치
    //
    Promise.resolve(null).then(() => {
      this.cy.on('tap', (e) => { 
        if( e.target === this.cy ){
          this.cyCanvasCallback(e);
          gtag('event', 'tap_canvas', {'event_category':'schema', 'event_label':'canvas'});
        } 
        else if( e.target.isNode() || e.target.isEdge() ){
          this.cyElemCallback(e.target);
          gtag('event', 'tap_element', {'event_category':'schema', 'event_label':'canvas'});
        }
      ...
      this.cy.on('doubleTap', (e, originalTapEvent) => {
        if( this.btnStatus.edit ){
          if( originalTapEvent.position ){
            this.createNode( originalTapEvent.position );
            gtag('event', 'new_node', {'event_category':'schema', 'event_label':'canvas'});
          } 
        }
      });
      ...
      this.cy.on('ehcomplete', (event, sourceNode, targetNode, addedEles) => {
        ...
        gtag('event', 'new_edge', {'event_category':'schema', 'event_label':'canvas'});
...
*/
  ...
  changeLayout( elements ){
    ...
    let options = { name: 'klay',
      ...
      priority: function( edge ){ return edge.data('size'); }, // null; }, // Edges with a non-nil value are skipped when geedy edge cycle breaking is enabled
      ...
      stop: () => this.saveSchema2Project() // on layoutstop           
    };    
  ...
  saveSchema2Project(){
    let project: IProject = {
      id: 0,                      // fixed id for SCHEMA Graph
      title: 'schema graph',
      description: this.dto_message,
      sql: '-- saved at '+moment().format("YYYY-MM-DD HH:mm:ss"),
      graph: null,
      image: this.cy.png({ full: true, scale: 0.5, maxWidth:300, maxHeight:400, bg: 'white' })
    };
    this.handlers[6] = this._api.mngr_project_save(project).subscribe(
      x => {
        if( x.hasOwnProperty('id') ) console.log(`schema image was saved at project.id=${x.id}`);
      });
  }
  ...
  onSelectTableLabels({ selected }){
    ...
    gtag('event', 'tap_row', {'event_category':'schema', 'event_label':'table'});
  }
  ...
  openConfirmDeleteLabelDialog(label:ILabel) {
    gtag('event', 'delete_label', {'event_category':'schema', 'event_label':'table'});
  ...
  saveElement(){
    gtag('event', 'save_label', {'event_category':'schema', 'event_label':'editor'});
